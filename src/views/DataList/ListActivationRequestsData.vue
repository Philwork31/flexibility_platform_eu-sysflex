                        ...
                        <div v-for="json in jsons" v-bind:class="{ modalActive: objectSelectedId == json.id }">
                            <div class="modal-container" v-if="objectSelectedId == json.id">
                                <div class="modal-header">
                                    <slot name="header">
                                        Selected activation request : Request from {{ json.system_operator_name }} at {{ json.start_of_delivery }} for {{ json.product_name }}
                                    </slot>
                                </div>
                    ...
                    <div class="tablerowwrapper">
                        <div class="tablerowwrapper-left">
                            <div class="tablerowwrappername">
                                <div>Request from {{ json.system_operator_name }} at {{ json.start_of_delivery }} for {{ json.product_name }}</div>
                            </div>
                            <div class="tablerowwrapperdetails" v-bind:class="{ nighttheme: $route.params.theme == 'n' }">
                                <div>Details : Quantity {{ json.quantity }}, Maximum price {{ json.maximum_price }} ...</div>
                            </div>
                        </div>
...
<script>
export default {
    name: "ListActivationRequestsData",
    data() {
        return {
            jsons: null,
            showModal: false,
    ...
    mounted() {
        var self = this;
        this.$axios
            .get(location.protocol + "//" + location.hostname+(location.port ? ":" + location.port: "") + '/api/v1/flexibilityrequestactivation/?format=json')
