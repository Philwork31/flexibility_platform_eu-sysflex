                        ...
                        <div v-for="json in jsons" v-bind:class="{ modalActive: objectSelectedId == json.id }">
                            <div class="modal-container" v-if="objectSelectedId == json.id">
                                <div class="modal-header">
                                    <slot name="header">
                                        Selected metering data : Metering data of {{ json.start_of_delivery_period }} from {{ json.fsp_name }} for {{ json.product_name }}
                                    </slot>
                                </div>
                    ...
                    <div class="tablerowwrapper">
                        <div class="tablerowwrapper-left">
                            <div class="tablerowwrappername">
                                <div>Metering data of {{ json.start_of_delivery_period }} from {{ json.fsp_name }} for {{ json.product_name }}</div>
                            </div>
...
export default {
    ...
    mounted() {
        var self = this;
        this.$axios
            .get(location.protocol + "//" + location.hostname+(location.port ? ":" + location.port: "") + '/api/v1/meteringdata/?format=json')
