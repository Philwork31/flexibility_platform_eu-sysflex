                    ...
                    <div class="tablerowwrapper">
                        <div class="tablerowwrapper-left">
                            <div class="tablerowwrappername">
                                <div>Activation order on product {{ json.product_name }}, for FSP {{ json.fsp_name }}</div>
                            </div>
                            <div class="tablerowwrapperdetails" v-bind:class="{ nighttheme: $route.params.theme == 'n' }">
                                <div>Details : Date {{ json.start_of_delivery }}, Localization factor {{ json.localization_factor}} ...</div>
                            </div>
                        </div>
...
export default {
    ...
    mounted() {
        var self = this;
        this.$axios
            .get(location.protocol + "//" + location.hostname+(location.port ? ":" + location.port: "") + '/api/v1/flexibilityactivationorder/?format=json')
